...
//2015-12-11 ueda
@property float osVersion;
@property Boolean upsideDownFlag;
@end 
...
//  [self updateOrientation];
    if (YES) {
        ...
        if (self.osVersion >= 9.0f) {
            UIInterfaceOrientation orientation = [[UIApplication sharedApplication] statusBarOrientation];
            if (orientation == UIInterfaceOrientationPortraitUpsideDown) {
                self.upsideDownFlag = YES;
            } else {
                self.upsideDownFlag = NO;
            }
...
- (void)updateOrientation
{
    ...
    switch (orientation) {
      case UIInterfaceOrientationPortraitUpsideDown:
        //2015-12-11 ueda
        //rotateAngle = M_PI;
            if ((self.osVersion >= 9.0f) && (self.upsideDownFlag)) {
                //NOP
            } else {
                rotateAngle = M_PI;
            }
